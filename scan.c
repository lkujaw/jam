...
/* This following header, which is generated by yacc, must come after
 * scan.h so that scan.h can properly define YYSTYPE.
 */
#include <jamgram.h>
...
static char *symdump PARAM((YYSTYPE *s));
...
yyfparse DECLARE((s))
    const char     *s  EP
BEGIN
    struct include *i = NIL(struct include *);

    memoryAllocateOrFail((voidT **)&i, sizeof(*i));
