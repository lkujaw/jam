# Run a command via CMD, and apply any environment difference in the
# CMD environment state to Git Bash
#
...
# Convert old path to associative array, where each entry has key =
# path, value = "1" Duplicate entries will be merged
...
# Convert new path to associative array, where each entry has key =
# path, value = "1" Duplicate entries will be merged
