    ...
    LIST       *result;
{
    ...
    if(rule->actions) {
        ...
        action->rule    = rule;
        action->targets = targetlist(_NIL_(TARGETS *), lol_get(args, 0));
        action->sources = targetlist(_NIL_(TARGETS *), lol_get(args, 1));
